...
void NewView(const std::function<void()>& viewFunc) {
    if (!animatingOverlay) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            totalH2 += h + (i?SPACING:0);
        }
        int y2 = (HEIGHT*SCALE - totalH2) / 2;
        for (int i = 0; i < m; i++) {
            int w,h;
            TTF_SizeUTF8(font, nxtDesc[i].label.c_str(), &w, &h);
            rect2[i] = {(WIDTH*SCALE - w)/2, y2, w, h};
            y2 += h + SPACING;
            ...
            tex2[i] = SDL_CreateTextureFromSurface(renderer, surf);
    ...
    while (running) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (curDesc[i].isButton) {
                ...
                if (st.animating) {
                    float dt = (SDL_GetTicks() - st.animStart) / ANIM_DURATION;
                    if (dt >= 1) { st.animating = false; st.alpha = NORMAL_ALPHA; }
                    else {
                        float v = springValues[size_t(dt * (springValues.size() - 1))];
